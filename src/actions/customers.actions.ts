...
const fetchCoordsByAddress = (address: string) => {
    ...
    return async (dispatch: any) => {
        try {
            ...
            const response = await axios.post(`/api/geocode/json?address=1600+Amphitheatre+Parkway,+Mountain+View,+CA&key=API_KEY`); //, loginQuery);
            ...
            dispatch({ type: FETCH_COORDS_ERROR, payload: { message: "Was not able to fetch coords. Please try again." } });
